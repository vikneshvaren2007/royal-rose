...
def init_db():
    ...
    with get_db() as conn:
        ...
        if cursor.fetchone()["cnt"] == 0:
            seed_products = [
                ("Classic Rose Milk", "Smooth, creamy and beautifully infused with authentic damask rose flavour.", 149.0, "images/Classic Rose Milk.jpg", "Whole Milk, Damask Rose Extract, Cane Sugar, Cardamom", "Rose Milk", "BESTSELLER", 1, 0, 1, 1, 4.9, 148),
                ("Coconut Rose Milk", "Delicate rose fragrance paired with velvety coconut milk for tropical luxury.", 179.0, "images/Rose Coconut Milk.jpg", "Fresh Coconut Milk, Rose Nectar, Saffron, Organic Sugar", "Rose Milk", "ORGANIC", 1, 0, 1, 1, 4.8, 94),
                ("Cardamom Rose Milk", "Fragrant cardamom crushed with sun-dried damask rose petals in pure milk.", 169.0, "images/Royal Rose Milk.jpg", "Pure Whole Milk, Green Cardamom, Damask Rose Syrup, Pistachio Bits", "Rose Milk", "ROYAL SPECIAL", 1, 0, 1, 1, 5.0, 215),
                ("Pistachio Rose Milk", "Roasted pistachio bits blended with saffron-infused royal rose essence.", 199.0, "images/Rose Chocolate.jpg", "Chilled Milk, Roasted Pistachios, Damask Rose Petals, Saffron", "Rose Milk", "SIGNATURE", 1, 0, 1, 1, 4.9, 176),
                ("Strawberry Rose Milk", "Sun-ripened strawberry puree swirled with traditional pink rose milk.", 169.0, "images/strawberry-rose.jpg", "Natural Strawberry Pulp, Rose Milk, Chia Seeds, Raw Honey", "Rose Milk", "POPULAR", 1, 0, 1, 1, 4.7, 110),
                ("Almond Rose Milk", "Rich crushed badam almonds steeped in aromatic chilled royal rose nectar.", 189.0, "images/Mango Rose Milk.jpg", "Mamra Almonds, A2 Whole Milk, Rose Essence, Saffron Strands", "Rose Milk", "CHEF'S CHOICE", 1, 0, 1, 1, 4.9, 142)
            ]
            cursor.executemany("""
...
def save_booking(customer_data, products_data, delivery_preference="Standard Delivery", payment_method="Cash on Delivery", notes=""):
    ...
    with get_db() as conn:
        ...
        for item in products_data:
            ...
            cursor.execute("SELECT id, name, price, available, sold_out, visible FROM products WHERE name = ? COLLATE NOCASE;", (p_name,))
...
def get_order_by_id_and_verification(order_id, verification):
    ...
    """
    clean_order_id = order_id.strip().upper()
    clean_verify = verification.strip().lower().replace(" ", "").replace("-", "")
    ...
    stored_phone = (order["customer"]["phone"] or "").strip().replace(" ", "").replace("-", "")

    if clean_verify != stored_email and clean_verify != stored_phone:
        return None
